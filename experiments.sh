...
do
	...
	do
		...
		java SerialQueueFirewall $EXECTIME $THREADS $W True $N $COUNTER
...
do
	echo "java SerialFirewall $EXECTIME $THREADS $W True $N $COUNTER"
	java ParallelFirewall $EXECTIME $THREADS $W True $N $COUNTER
	echo "Iteration $COUNTER: " >> $OUTFILE
	COUNTER=$[$COUNTER +1]
done
...
do
	...
	do
		echo "java SerialFirewall $EXECTIME $THREADS $W True $N $COUNTER"
		java SerialFirewall $EXECTIME $THREADS $W False $N $COUNTER
		echo "Iteration $COUNTER: " >> $OUTFILE
		COUNTER=$[$COUNTER +1]
	done
done
